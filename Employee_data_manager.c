...
// Loads the Employee List from disk (File I/O and Dynamic Allocation)
void load_employees(Employee **head) {
    ...
    fclose(file);
    printf("Employees loaded from %s\n", "employees.csv");
}
...
void load_departments(Department **head) {
    ...
    fclose(file);
    printf("Departments loaded from %s\n", "departments.csv");
}
...
JoinedRecord* create_joined_record(Employee *emp, Department *dept) {
    ...
    if (!new_record->employee_name || !new_record->department_name) { 
        free(new_record->employee_name); free(new_record->department_name); free(new_record); 
        return NULL; 
    }
    return new_record;
}
...
void display_joined_results(JoinedRecord *head) {
    ...
    printf("-----|----------------------|--------------|--------------------\n");
    if (current == NULL) {
        printf("No matching records found in the join\n");
    }
    while (current != NULL) {
...
void add_department_cli(Department** head) {
    ...
    if (scanf("%s", name) != 1) { while (getchar() != '\n');
         return;
         }
